"""
visualize_copilot_v2.py
=======================
V2 improvements over V1:
  - Coordinates displayed in pixel space (radius 432) matching README
  - Keyboard labels shown on each panel (all 8 directions labeled,
    target direction highlighted)
  - Single accuracy annotation per panel (no duplicate)
  - Circular subplot layout: panels arranged in compass positions
  - Evaluation matches evaluate.py exactly (cross-trial history per subject)
    so per-direction numbers align with the official +0.7pp result
  - "Show Correction" button per panel: animates a real trial where the
    BCI decoder was wrong but BCI+Copilot corrected it (or vice versa)

Run from arm-bci-copilot/:
    python visualize_copilot_v2.py

Output: copilot_visualization_v2.html  (~8MB, self-contained, works offline)
"""
...
OUTPUT_PATH = "copilot_visualization_v2.html"
...
# chargeTargets target positions (sorted by x,y — matches Run5 training)
_all_targets   = LABEL_TO_DIR.tolist() + [[0.0, 0.0]]
TARGETS_SORTED = np.array(sorted(_all_targets, key=lambda p: (p[0], p[1])), dtype=np.float32)
...
def vel_to_softmax(vx, vy):
    ...
    if spd > 1e-6: vx /= spd; vy /= spd
    s = np.zeros(5, dtype=np.float32)
    s[1]=max(vx,0.); s[0]=max(-vx,0.); s[2]=max(vy,0.); s[3]=max(-vy,0.)
    return s

def softmax_to_vel(s):
    return np.clip(np.array([s[1]-s[0], s[2]-s[3]], dtype=np.float32), -1, 1)

def calc_copilot_vel(action, cursor):
    charges = (action + 1.0) / 2.0
    charges = torch.softmax(torch.tensor(charges/TEMPERATURE, dtype=torch.float32), dim=0).numpy()
    diffs = TARGETS_SORTED - cursor
...
class HistQueue:
    ...
    def reset(self, pos=None):
        self.data[:] = pos if pos is not None else 0.
        self.i = 0
...
def build_obs(cursor, vel, hq):
    base = np.concatenate([cursor, vel]).astype(np.float32)
    hist = hq.add_get(cursor)
...
def sim_copilot(model, trial, hq, prev):
    """BCI+Copilot trajectory. Returns (T+1,2) normalized."""
    hq.reset(pos=prev)
    cursor = np.zeros(2, dtype=np.float32)
    path   = [cursor.copy()]
    ep     = True; _st = None
    for vx, vy in trial['vel_seq']:
        enc = softmax_to_vel(vel_to_softmax(vx, vy))
        obs = build_obs(cursor, enc, hq)
        act, _st = model.predict(obs, state=_st, deterministic=True, episode_start=ep)
        ep = False
        cursor = np.clip(cursor + np.array([vx,vy],dtype=np.float32) + calc_copilot_vel(act,cursor), -1.5, 1.5)
        path.append(cursor.copy())
    return np.array(path)
...
def compute_all(model, by_dir, by_subj):
    ...
    # Run copilot on all trials in subject order (matches evaluate.py)
...
def build_figure(results):
    ...
    annotations.append(dict(
        ...
        text=(f'Run5 model (chargeTargets, ent_coef=0.0) · '
              f'Data: online_arm_trajectories.csv · '
              f'Overall: BCI 46.7% → Copilot 47.4% (+0.7pp) · '
...
<h2>Run5 model (chargeTargets, ent_coef=0.0) &nbsp;·&nbsp;
    Data: online_arm_trajectories.csv &nbsp;·&nbsp;
    Overall: BCI 46.7% → Copilot 47.4% (+0.7pp)</h2>
...
def main():
    print("=" * 60)
    print("BCI Copilot Visualization V2")
    print("=" * 60)
